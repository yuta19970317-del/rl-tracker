...
function calcEscapeWins(record: PlayerRecord, escapeTarget: number | null): number | null {
...
type Props = {
  // 対象者フィルタ済みかつ minMatches 適用済みの records
  records: PlayerRecord[];
  players: Player[];
};

export function EkibyoCard({ records, players }: Props) {
  ...
  const minWinRate = Math.min(...records.map((r) => r.winRate));
  const ekibyo = records.filter((r) => r.winRate === minWinRate);

  let escapeWins: number | null = null;
  if (ekibyo.length === 1) {
    const sorted = [...records].sort((a, b) => a.winRate - b.winRate);
  ...
  return (
    <div className="bg-black border-2 border-red-700 rounded-xl p-5 space-y-4 shadow-lg shadow-red-900/30">
      ...
      <div className="space-y-3">
        {ekibyo.map((r) => (
          <div key={r.playerId} className="bg-red-950/30 border border-red-800/50 rounded-lg p-4">
            <div className="flex items-center justify-between mb-3">
              <span className="text-white font-black text-lg">⚠️ {name(r.playerId)}</span>
              <span className="text-red-400 font-black text-2xl">{pct(r.winRate)}</span>
            </div>
